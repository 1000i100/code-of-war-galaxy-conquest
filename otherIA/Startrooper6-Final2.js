...
var getOrders = function(context) {
	...
	var ennemyFleet = getEnnemyFleet(id, context);
	...
	if ( otherPlanets != null && otherPlanets.length > 0 )
	{
		...
		for ( var i = 0; i<myPlanets.length; i++ )
		{
			...
			var myPlanet = myPlanets[ i ];
			var Cible = getNearestPlanet(myPlanets[myPlanets.length-1],otherPlanets);
			var bouillaveLe = otherPlanets;
			var maMaison = myPlanets [0];
			
			var Distance = GameUtil.getDistanceBetween( new Point( myPlanet.x, myPlanet.y ), new Point( Cible.x, Cible.y ) );

			


			if(myPlanets.length == 1){
				if(myPlanet.size != PlanetSize.SMALL){
				if(myPlanet.population >= 80){
					if(Cible.size == PlanetSize.SMALL)
						result.push(new Order(myPlanet.id,Cible.id, 70));
					if(Cible.size == PlanetSize.NORMAL)
						result.push(new Order(myPlanet.id,Cible.id, 80));
					if(Cible.size == PlanetSize.BIG)
						result.push(new Order(myPlanet.id,Cible.id, 80));
					if(Cible.size == PlanetSize.HUGE)
						result.push(new Order(myPlanet.id,Cible.id, 80));
				}
				}
...
/*
				...
				if(myPlanets[0].population + myPlanets[1].population >= 40){
					if(Cible.size == PlanetSize.SMALL)
						result.push(new Order(myPlanet.id,Cible.id, myPlanet.population ));
					if(Cible.size == PlanetSize.NORMAL)
						result.push(new Order(myPlanet.id,Cible.id, myPlanet.population));
					if(Cible.size == PlanetSize.BIG)
						result.push(new Order(myPlanet.id,Cible.id, myPlanet.population));
					...
					result.push(new Order(myPlanet.id,Cible.id, 3 ));

			}
*/
			if(myPlanets.length >= 2){
							
							var planeteOrigine = myPlanets;
							var premInvasion = otherPlanets;
					
					result.push( new Order( myPlanet.id, getNearestPlanet(planeteOrigine,premInvasion).id, myPlanet.population) );
...
var getEnnemyFleet = function ( sourceID, candidats){
	var result = [];
	for ( var i = 0; i<candidats.length; i++ )
		{
			var element = candidats[ i ];
			if ( element.owner.id != sourceID)
			{
				result.push(element);
			}
