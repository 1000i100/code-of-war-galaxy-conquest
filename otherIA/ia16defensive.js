...
MyIA.prototype = $extend(WorkerIA.prototype,{
	...
	,strategy: function() {
		return new strategy.Spread(this);
	}
});
var Std = function() { }
Std.__name__ = true;
...
strategy.Germany = function(level,quant) {
...
strategy.Random.prototype = {
	getOrders: function(context,id) {
		...
		while(_g < myPlanets.length) {
			...
			while(_g2 < _g1) {
				var i = _g2++;
				orders.push(new com.tamina.planetwars.data.Order(source.id,enemyPlanets[Std.random(enemyPlanets.length)].id,1));
			}
		}
		return orders;
	}
	...
	,getOrders: function(context,id) {
		...
		while(_g < myPlanets.length) {
			...
			enemyPlanets.sort((function(source) {
				return function(a,b) {
					return com.tamina.planetwars.utils.GameUtil.getDistanceBetween(new com.tamina.planetwars.geom.Point(a.x,a.y),new com.tamina.planetwars.geom.Point(source[0].x,source[0].y)) - com.tamina.planetwars.utils.GameUtil.getDistanceBetween(new com.tamina.planetwars.geom.Point(b.x,b.y),new com.tamina.planetwars.geom.Point(source[0].x,source[0].y)) | 0;
				};
			})(source));
			var i = 0;
			var _g1 = 0;
			while(_g1 < enemyPlanets.length) {
				...
				++_g1;
				i++;
				var populationByLanding = Math.min(target.population + com.tamina.planetwars.utils.GameUtil.getTravelNumTurn(source[0],target) * 5,com.tamina.planetwars.data.PlanetPopulation.getMaxPopulation(target.size)) | 0;
				if(source[0].population > populationByLanding + 1 && target.population > 0) {
					source[0].population -= populationByLanding + 1;
					target.population -= populationByLanding + 1;
					var order = this.newOrder(source[0],target,populationByLanding + 1);
					this.ia.debugMessage = "test : " + i;
...
strategy.StraightToCore = function(percentage) {
