...
var getOrders = function(context) {
	...
	if ( otherPlanets != null && otherPlanets.length > 0 )
	{
		...
		{
			var sortedPlanets = sortPlanets(otherPlanets);
			var myPlanet = myPlanets[ i ];
			var pussanceAttack = myPlanet.population ;
			if(myPlanet.population >= 40 ){
				var toAttack = sortedPlanets[0];
				if ( myPlanet.population > toAttack.population + 5 ) {
					pussanceAttack += ( toAttack.population - myPlanet.population ) + 5;
					toAttack = sortedPlanets[2];
				}
				debugMessage = "attack de "+pussanceAttack+" sur "+toAttack.population;
				result.push( new Order( myPlanet.id, toAttack.id, pussanceAttack ) );
			}
		}
	}
	return result;
...
var sortPlanets = function ( planets ) {
	return planets.sort(function ( planet1 , planet2 ) { return planet1.population - planet2.population});
}
var getSmallers = function ( myplanet, candidats ) {
...
var getSmallerPlanet = function ( candidats ) {
		var result = candidats[ 0 ];
		for ( var i = 0; i<candidats.length; i++ ) {
...
var getBigerPlanet = function ( candidats ) {
		var result = candidats[ 0 ];
...
var getSmallerPlanetAtDistance = function ( distance , candidats  ) {
		var result = candidats[ 0 ];
