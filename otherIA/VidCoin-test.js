...
var getOrders = function(context) {
	...
	if ( otherPlanets != null && otherPlanets.length > 0 )
	{
		for ( var i = 0; i<myPlanets.length; i++ )
		{
			...
			var populationToSend = 40;
			if(myPlanet.population >=populationToSend){
				result.push( new Order( myPlanet.id, attackNearestOrFurthestPlanet(myPlanet,otherPlanets).id, populationToSend/2 ) );
			}
...
var attackNearestOrFurthestPlanet = function( source, candidats )
{
	var rand = Math.floor((Math.random()*10)+1);
	if(rand > 5) {
		return getNearestPlanet( source, candidats );
	}
	else {
		return getFurthestPlanet( source, candidats );
	}
};
