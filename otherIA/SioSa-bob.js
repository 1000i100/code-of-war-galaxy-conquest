...
MyIA.prototype = $extend(WorkerIA.prototype,{
	existsNeutralPlanet: function(ennemyPlanets,candidats) {
		var i = 0;
		while(i <= candidats.length) {
			var _g1 = 0, _g = ennemyPlanets.length;
			while(_g1 < _g) {
				var j = _g1++;
				if(candidats[i] == ennemyPlanets[j]) HxOverrides.remove(candidats,candidats[i]); else i++;
			}
		}
		if(candidats == null) return null; else return candidats[0];
	}
	,isFull: function(target) {
		if(target.population >= 50 && target.size == 1) return true; else if(target.population >= 100 && target.size == 2) return true; else if(target.population >= 200 && target.size == 3) return true; else if(target.population >= 300 && target.size == 4) return true; else return false;
	}
	,maxPopulation: function(target) {
		if(target.size == 1) return 50; else if(target.size == 2) return 100; else if(target.size == 3) return 200; else return 300;
	}
	,canIWin: function(source,target) {
		if(source.population >= 71 && target.size == 1) return 51; else if(source.population >= 121 && target.size == 2) return 101; else if(source.population >= 221 && target.size == 3) return 201; else if(source.population == 300 && target.size == 4) return 300; else return 0;
	}
	,getMaisonMere: function(source,candidats) {
	...
	,getOrderSizeEnnemyPlanet: function(source,candidats) {
		var x;
		var _g = 1;
		while(_g < 4) {
			var k = _g++;
			var _g2 = 0, _g1 = candidats.length;
			while(_g2 < _g1) {
				var i = _g2++;
				var _g4 = 1, _g3 = candidats.length;
				while(_g4 < _g3) {
					var j = _g4++;
					if(candidats[j].size == k && com.tamina.planetwars.utils.GameUtil.getDistanceBetween(new com.tamina.planetwars.geom.Point(source.x,source.y),new com.tamina.planetwars.geom.Point(candidats[j].x,candidats[j].y)) <= com.tamina.planetwars.utils.GameUtil.getDistanceBetween(new com.tamina.planetwars.geom.Point(source.x,source.y),new com.tamina.planetwars.geom.Point(candidats[i].x,candidats[i].y))) {
						x = candidats[j];
	...
	,getNearestEP1: function(source,candidats) {
		var result = new Array();
		var dist = 16.66;
		var _g1 = 0, _g = candidats.length;
		while(_g1 < _g) {
			var i = _g1++;
			var currentDist = com.tamina.planetwars.utils.GameUtil.getDistanceBetween(new com.tamina.planetwars.geom.Point(source.x,source.y),new com.tamina.planetwars.geom.Point(candidats[i].x,candidats[i].y));
			if(currentDist <= dist) {
				var element = candidats[i];
				result.push(element);
			}
	...
	,getOrders: function(context) {
		...
		var maisonMere = myPlanets[0];
		var coupFatal = this.getMaisonMere(maisonMere,otherPlanets);
		var ennemyPlanets;
		var existsNP;
		var attaque = 30;
		if(otherPlanets != null && otherPlanets.length > 0) {
			MyIA.comptTour++;
			if(MyIA.comptTour == 1) {
				MyIA.ennemyOwner = coupFatal.owner;
				attaque = 100;
			} else attaque = 30;
			ennemyPlanets = com.tamina.planetwars.utils.GameUtil.getEnnemyPlanets(MyIA.ennemyOwner.id,context);
			existsNP = this.existsNeutralPlanet(ennemyPlanets,otherPlanets);
			if(MyIA.comptTour == 1) {
			} else if(existsNP != null) MyIA.cible = existsNP; else MyIA.cible = otherPlanets[0];
			var _g1 = 0, _g = myPlanets.length;
			while(_g1 < _g) {
				var i = _g1++;
				var myPlanet = myPlanets[i];
				var target = this.getNearestEnnemyPlanet(myPlanet,otherPlanets);
				var candidats = this.getOrderSizeEnnemyPlanet(myPlanet,otherPlanets);
				if(myPlanet.population > 45) {
					MyIA.cible = otherPlanets[0];
					result.push(new com.tamina.planetwars.data.Order(myPlanet.id,MyIA.cible.id,attaque));
				}
			}
		}
		return result;
