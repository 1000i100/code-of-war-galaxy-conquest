...
// Déclencheur pour les différentes phases de l'ia
var phaseNumber = 1
...
var getOrders = function(context) {
  ...
  var otherPlanets = GameUtil.getEnnemyPlanets(id, context);
   debugMessage= "GET OTDERS" + JSON.stringify(otherPlanets);
  // On identifie les petites planètes

  // On regarde la planète la plus proche


  if (otherPlanets != null && otherPlanets.length > 0)
  {
    // Pour chacunes de nos planètes
    for (var i = 0; i < myPlanets.length; i++)
    {
      var myPlanet = myPlanets[i];
      var nearestSmallPlanets = getPlanetsSortedByNearest(myPlanet, otherPlanets);

      if(myPlanet.population >= PlanetPopulation.getMaxPopulation(myPlanet.size) / 2  && nearestSmallPlanets.length > 0)
      {
        orders.push( new Order(myPlanet.id, nearestSmallPlanets[0].id, myPlanet.population / 2) );
      }

      else
      {
        debugMessage= "MOI YAIME LA TECHA";
      }
...
// FONCTION CUSTOM pour avoir un tableau des planètes les plus proches par rapport à une certaine planète
var getPlanetsSortedByNearest = function(source, planets, filter)
{
  // S'il y a plus d'une planète ennemie, sinon ça sert à rien de trier et ça bug et dans tous les cas on la défonce
  if(source =! null && planets.length < 1)
  {
    var i = 0;
    while(i < planets.length)
    {
      debugMessage= JSON.stringify(planets[i]);
      if( GameUtil.getDistanceBetween(new Point(source.x, source.y), new Point(planets[i].x, planets[i].y)) < GameUtil.getDistanceBetween(new Point(source.x, source.y), new Point(planets[i+1].x, planets[i+1].y)) )
      {
        i++;
      }
  ...
  {
     debugMessage  = "PIPI LOL";
  }
  // On filtre les planètes si le paramètre est mentionné
  if(filter)
  {
    for(i = 0; i < planets.length; i++)
    {
      if(planets[i].size != filter)
      {
        debugMessage= "SUCE MA QUZUZ";
        planets.unset(planets[i])
      }
    }
  }
