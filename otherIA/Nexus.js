...
var COEFF_RANGE = 50; //Poids de la range dans le choix de la planète à attack
var COEFF_POP = 1; //Poids de la pop dans la planète à attaquer

var NB_PLANETS_TO_SCORE = 3;
var SCORE_GAME = false;
var END_THIS = false;
var RATIO_END = 0.55;
var RATIO_POP_SCORE = 1.25;
var RATIO_NB_PLANET = 3;
var MAX_TURN = 150;
...
var getOrders = function(context) 
{
	...
	checkScoreGame(context,my_planets,other_planets);
	checkEnd(context,my_planets,other_planets);
...
var checkScoreGame = function(context,my_planets,other_planets)
{
	SCORE_GAME = false;
	if (!SCORE_GAME) 
	{
		var cpt = 0;
		var totalPopEnnemy = 0;
		for (var i = other_planets.length - 1; i >= 0; i--) 
		{
			if (other_planets[i].owner.id == ennemy_id)
			{
				totalPopEnnemy += other_planets[i].population;
				cpt++;
			}
		};
		if (cpt == 1)
		{
			...
			{
				myTotalPop += my_planets[i].population;
				cpt++;
			};
			if (cpt > context.content.length/RATIO_NB_PLANET && myTotalPop >= totalPopEnnemy*RATIO_POP_SCORE)
			{
				SCORE_GAME = true;
			}
		}
...
var checkEnd = function(context,my_planets,other_planets)
{
	END_THIS = false;
	if (SCORE_GAME) 
	{
		...
		if (cpt == 0) // Plus de planètes neutres
		{
			var myTotalPop = 0;
			var totalPossible = 0;
			for (var i = my_planets.length - 1; i >= 0; i--) 
			{
				myTotalPop += my_planets[i].population;
				totalPossible += PlanetPopulation.getMaxPopulation(my_planets[i].size);
			};

			totalPossible *= 2;

			for (var i = context.fleet.length - 1; i >= 0; i--) {
				if(context.fleet[i].id == id)
				{
					myTotalPop += context.fleet[i].crew;
				}
			};
			if (myTotalPop >= RATIO_END*totalPossible || current_turn >= MAX_TURN)
			{
				setMessage("END_THIS");
				END_THIS = true;
				SCORE_GAME = false;
...
var AttackWeakestPlanet = function(context,my_planets)
{
	...
	for (var i = other_planets.length - 1; i >= 0; i--) 
	{
		if (SCORE_GAME && other_planets[i].owner.id != ennemy_id) // Onrécupère uniquement les planètes les plus faibles
		{
			potentials_targets.push(other_planets[i]);
		}
		else if (!SCORE_GAME && !END_THIS && bait !=null && bait.id != other_planets[i].id)
		{
			potentials_targets.push(other_planets[i]);
		}
		else if (!SCORE_GAME)
...
var getHelp = function(planet, context, my_planets, amount,turn)
{
	...
	/*setMessage(planet);
	setMessage(planet.size == 1);*/
	if(SCORE_GAME && planet.size == 1 && bait == null)
	{
		/*setMessage("NBAIT");
		setMessage(planet);*/
		bait = planet;
