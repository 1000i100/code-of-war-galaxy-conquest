...
var getOrders = function(context) {
    ...
    {
        var myPlanet = myPlanets[ 0 ];
        var baseAdverse = otherPlanets[0];
        if (myPlanet.population - attackNumber >= baseAdverse.population) {
            result.push(new Order(myPlanet.id, baseAdverse.id, myPlanet.population - attackNumber));
        }
        for (var i = 0; i < myPlanets.length; i++)
        {
            var myPlanet = myPlanets[ i ];
            var target = getSmallestPlanet(myPlanet, otherPlanets);
            if (myPlanet.population >= target.population + 10) {
                result.push(new Order(myPlanet.id, target.id, target.population + 10));
            }
...
var getSmallestPlanet = function(source, candidats)
{
    var result = candidats[ 0 ];
    for (var i = 0; i < candidats.length; i++)
    {
