...
strategy.Expand = function(distance,marge,level) {
	if(level == null) level = 20;
	if(marge == null) marge = 0.5;
		...
		if(otherPlanets != null && otherPlanets.length > 0) {
			...
			while(_g < myPlanets.length) {
				...
				while(planetsAround.length == 0) {
					planetsAround = this.getPlanetsAround(myPlanet[0],this.distance + acc,this.marge,otherPlanets);
					acc++;
				}
				var quant = new Array();
				var sum = 0;
				...
				while(_g1 < planetsAround.length) {
					...
					++_g1;
					sum += planet.size;
					dbgstr += planet.toString() + "\n";
				}
				if(myPlanet[0].population >= this.level) {
					var _g2 = 0, _g1 = planetsAround.length;
					while(_g2 < _g1) {
						var i = _g2++;
						var populationByLanding = planetsAround[i].population + com.tamina.planetwars.utils.GameUtil.getTravelNumTurn(myPlanet[0],planetsAround[i]) * 5;
						var evenpop = Math.floor(myPlanet[0].population / sum * planetsAround[i].size);
						var maxpop = com.tamina.planetwars.data.PlanetPopulation.getMaxPopulation(planetsAround[i].size);
						var toSend = maxpop < evenpop?maxpop + 1:evenpop;
						result.push(new com.tamina.planetwars.data.Order(myPlanet[0].id,planetsAround[i].id,toSend));
					}
				}
			}
		}
