...
MyIA.prototype = $extend(WorkerIA.prototype,{
	...
	,getOrders: function(context) {
		var result = new Array();
	    var ennemyPlanets = com.tamina.planetwars.utils.GameUtil.getEnemyPlanets(this.id, context);
        if (ennemyPlanets == null || ennemyPlanets.length == 0)
            return result;
        if (this.path == null) { // first turn
	        var myPlanets = com.tamina.planetwars.utils.GameUtil.getPlayerPlanets(this.id, context);
            this.path = [myPlanets[0].id];
            this.targetId = this.getNextTarget(context).id;
        }
        //var newPath = [];
        //for (var i = 0; i < this.path.length; i++) {
        //    var planet = com.tamina.planetwars.utils.GameUtil.getPlanet(this.path[i], context);
        //    if (planet.owner.id == this.id) // elle n'est pas focrement perdu grace éventuels troupes qui arrivent
        //        newPath.push(planet.id);
        //    //else reprendre la planet perdu ?
        //}
        //this.path = newPath;
        if (this.target(context).owner.id == this.id) {
            this.path.push(this.targetId);
            this.targetId = this.getNextTarget(context).id;
        }
        //this.storeShips(context);
        for (var i = 0; i < this.path.length - 1; i++) {
            var sourcePlanet = com.tamina.planetwars.utils.GameUtil.getPlanet(this.path[i], context);
            if (sourcePlanet.owner.id != this.id) break;
            var targetPlanet = com.tamina.planetwars.utils.GameUtil.getPlanet(this.path[i + 1], context);
            var numUnits = Math.floor(sourcePlanet.population / 2.5);
            //var estimatePopulation = this.estimatePopulation(sourcePlanet, context);
            //if (estimatePopulation - numUnits < 0)
            //    numUnits = numUnits - estimatePopulation;
            if (numUnits < sourcePlanet.population && numUnits > 0) {
                result.push(new com.tamina.planetwars.data.Order(sourcePlanet.id, targetPlanet.id, numUnits));
            }
        }
        var sourcePlanet = this.pathHead(context);
        if (sourcePlanet != null && sourcePlanet.owner.id == this.id) {
            var numUnits = Math.floor(sourcePlanet.population / 2);
            //var estimatePopulation = this.estimatePopulation(sourcePlanet, context);
            //if (estimatePopulation - numUnits < 0)
            //    numUnits = numUnits - estimatePopulation;
            if (numUnits < sourcePlanet.population && numUnits > 5) {
                result.push(new com.tamina.planetwars.data.Order(sourcePlanet.id, this.targetId, numUnits));
            }
        }
        this.currentTurn++;
		return result;
    ...
    target: function(context) {
        return com.tamina.planetwars.utils.GameUtil.getPlanet(this.targetId, context);
    },
    pathHead: function(context) {
        if (this.path.length > 0)
            return com.tamina.planetwars.utils.GameUtil.getPlanet(this.path[this.path.length - 1], context);
    },
    myPathHead: function(context) {
        for (var i = this.path.length - 1; i >= 0; i--) {
            var planet = this.path[i];
            if (planet.owner.id == this.id)
                return planet
        }
    },
    pathRoot: function(context) {
        if (this.path.length > 0)
            return com.tamina.planetwars.utils.GameUtil.getPlanet(this.path[0], context);
    },
    planetInPath: function(planet) {
        for (var i = 0; i < this.path.length; i++) {
            if (planet.id == this.path[i].id)
                return true;
        }
        return false;
    },
    getNextTarget: function(context) {
        var headPlanet = this.pathHead(context);
        var enemyPlanets = com.tamina.planetwars.utils.GameUtil.getEnemyPlanets(this.id, context);
        var nearestEnemyPlanet = this.getNearestPlanet(headPlanet, enemyPlanets);
        var otherPlanets = com.tamina.planetwars.utils.GameUtil.getOtherPlanets(this.id, context);
        var nextTargetData = null;
        for (var i = 0; i < otherPlanets.length; i++) {
            var planet = otherPlanets[i];
            if (this.planetInPath(planet)) continue;
            var planetData = {
                planet: planet,
                distToHead: com.tamina.planetwars.utils.GameUtil.getDistanceBetweenPlanets(planet, headPlanet),
                distToEnemy: com.tamina.planetwars.utils.GameUtil.getDistanceBetweenPlanets(planet, nearestEnemyPlanet),
            };
            planetData.score = planetData.distToHead * 1.5 + planetData.distToEnemy + planet.population;
            if (nextTargetData == null || nextTargetData.score > planetData.score)
                nextTargetData = planetData;
        }
        return nextTargetData.planet;
    },
    storeShips: function(context) {
        this.shipsStore = {};
        //// Method 1
        //for (var i = 0; i < context.fleet.length; i++) {
        //    var ship = context.fleet[i];
        //    var turn = ship.travelDuration - (this.currentTurn - ship.creationTurn);
        //    if (this.shipsStore[ship.target.id] == null)
        //        this.shipsStore[ship.target.id] = {nbShips: 0};
        //    this.shipsStore[ship.target.id][turn] = ship;
        //    this.shipsStore[ship.target.id].nbShips++;
        //}
        ////Method 2
        //for (var i = 0; i < context.fleet.length; i++) {
        //    var ship = context.fleet[i];
        //    if (this.shipsStore[ship.target.id] == null)
        //        this.shipsStore[ship.target.id] = [];
        //    this.shipsStore[ship.target.id].push(ship);
        //}
    },
    estimatePopulation: function(planet, context) {
        var population = planet.population;
        //// Method 1 => prend trop de temps
        //var shipsByTurn = this.shipsStore[planet.id];
        //if (shipsByTurn == null) return population;
        //var nbShipsComputed = 0;
        //var turn = 1;
        //while (nbShipsComputed < shipsByTurn.nbShips - 1) {
        //    population += 5;
        //    var ship = shipsByTurn[turn];
        //    if (ship != null) {
        //        if (ship.owner.id == this.id)
        ...
        //    turn++;
        //}
        //// Medthod 2 => mauvais calcul
        //var ships = this.shipsStore[planet.id];
        //if (ships == null) return population;
        //for (var i = 0; i < ships.length; i++) {
        //    if (ships[i].owner.id == this.id) {
        //        population += ships[i].crew;
        //    }
        //    else {
        //        population -= ships[i].crew;
        //    }
...
com.tamina.planetwars.utils.GameUtil.getDistanceBetweenPlanets = function(planet1,planet2) {
	return com.tamina.planetwars.utils.GameUtil.getDistanceBetween(new com.tamina.planetwars.geom.Point(planet1.x,planet1.y),new com.tamina.planetwars.geom.Point(planet2.x,planet2.y));
}
...
com.tamina.planetwars.utils.GameUtil.getEnemyFleet = function(playerId,context) {
	var result = new Array();
...
com.tamina.planetwars.utils.GameUtil.getOtherPlanets = function(planetOwnerId,context) {
	var result = new Array();
	var _g1 = 0, _g = context.content.length;
	while(_g1 < _g) {
		var i = _g1++;
...
com.tamina.planetwars.utils.GameUtil.getEnemyPlanets = function(planetOwnerId,context) {
	...
	while(_g1 < _g) {
		var i = _g1++;
		var p = context.content[i];
        if(p.owner.id != planetOwnerId && p.owner.id <= 2)
            result.push(p);
	}
	return result;
}
com.tamina.planetwars.utils.GameUtil.getPlanet = function(id,context) {
