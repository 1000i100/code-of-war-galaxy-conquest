...
(function() {
    var Galaxy, Game, GameUtil, Order, Planet, PlanetPopulation, PlanetSize, Point, Range, Ship, TurnMessage, TurnResult, UID, color, debugMessage, getNearestPlanet, getOrders, getPopulationAfterTurns, getTravelDurationBetween, id, name;
    ...
    getOrders = function(context) {
        var myPlanet, myPlanets, neareastPlanet, otherPlanets, result, _i, _len, populationAfterTurns;
        result = [];
        ...
        if (otherPlanets !== null && otherPlanets.length > 0) {
            for (_i = 0, _len = myPlanets.length; _i < _len; _i++) {
                ...
                var bestPlanet = getBestPlanet(myPlanet, otherPlanets);
                populationAfterTurns = getPopulationAfterTurns(bestPlanet, getTravelDurationBetween(myPlanet, bestPlanet));
                var populationToSend = 0;
                debugMessage = "MyPopulation: " + myPlanet.population + " -- Other population after: " + populationAfterTurns;
                if(myPlanet.population > populationAfterTurns + Game.PLANET_GROWTH) {
                    populationToSend = populationAfterTurns + Game.PLANET_GROWTH;
                }
                else {
                    populationToSend = myPlanet.population;
                }
                result.push(new Order(myPlanet.id, bestPlanet.id, populationToSend));
            }
        }
        return result;
    };
    ...
    getTravelDurationBetween = function(source, target) {
        return Math.ceil(GameUtil.getDistanceBetween(new Point(source.x, source.y), new Point(target.x, target.y)) / Game.SHIP_SPEED)
    };

    getPopulationAfterTurns = function(planet, nbTurns) {
        return planet.population + nbTurns * Game.PLANET_GROWTH;
    };

    getOtherPlanetsPoints = function(source, others) {
        var otherPlanets = [];
        for(var i = 0; i< others.length; i++) {
            var currentPlanet = others[i];
            var planetPoints = getTravelDurationBetween(source, currentPlanet) + currentPlanet.population/10;
            otherPlanets.push(new PlanetPoint(currentPlanet, planetPoints));
        }
        return otherPlanets;
    };

    getBestPlanet = function(source, others) {
        var otherPlanetsPoints = getOtherPlanetsPoints(source, others);
        var bestPlanet;
        for(var i= 0; i<otherPlanetsPoints.length; i++) {
            if(!bestPlanet || otherPlanetsPoints[i].points < bestPlanet.points) {
                bestPlanet = otherPlanetsPoints[i];
            }
        }
        return bestPlanet.planet;
    }

    PlanetPoint = (function() {
        function PlanetPoint(planet, points) {
            this.planet = planet;
            this.points = points;
        }
...
/*
 //@ sourceMappingURL=basic_IA-cs.map
 */
