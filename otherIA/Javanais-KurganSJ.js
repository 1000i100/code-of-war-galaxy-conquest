...
com.tamina.planetwars.utils.GameUtil.getNewPosition2 = function(currentGalaxy) {
	var result = new com.tamina.planetwars.geom.Point(0.0,0.0);
	var isFarEnough = false;
	while(!isFarEnough) {
		result = new com.tamina.planetwars.geom.Point(Math.random() * (currentGalaxy.width - 140) + 70,Math.random() * (currentGalaxy.height - 140) + 70);
		isFarEnough = true;
		var _g1 = 0, _g = currentGalaxy.content.length;
		while(_g1 < _g) {
			var i = _g1++;
			var p = currentGalaxy.content[i];
			if(com.tamina.planetwars.utils.GameUtil.getDistanceBetween(result,new com.tamina.planetwars.geom.Point(p.x,p.y)) < 70) isFarEnough = false;
		}
	}
	return result;
}
